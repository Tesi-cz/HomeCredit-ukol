...
přepsané ani zkrácené — celé znění je autoritativně uložené v repozitáři:

➡️ soubor `Domácí_úkol_—_AI_Implementation_Expert_(zadání).md` v kořeni repozitáře

Tento soubor obsahuje jeho plné znění (viz níže), aby byl Master Prompt čitelný
přímo ze složky `prompts/`.
...
Zadání se nepředalo jako jediný „vygeneruj mi aplikaci" prompt. Bylo výchozím
bodem, ze kterého vznikla řízená sada specifikací (požadavky → návrh → rozhraní →
databáze → plán úkolů), a teprve podle nich probíhala implementace. Prompty pro
jednotlivé kroky jsou v sousedních souborech (`10-pozadavky.md` až
`50-plan-a-implementace.md`).

---

## Plné znění zadání (Master Prompt)
